<?php
	...
	if(isset($_POST['buscar']) && $_POST['buscar'] == 'Buscar'){
		...
		if(isset($_POST['cpf']) && strlen($_POST['cpf']) == 11){
			...
			$r= tentaSlcCliente($cpf);
			
			$r = mysql_fetch_assoc($r);

			echo json_encode($r);
			return ;
		}else if(isset($_POST['cnpj']) && strlen($_POST['cnpj']) == 14){
			$cnpj = addslashes($_POST['cnpj']);
			$r= tentaSlcFabricante($cnpj);
			...
			$codigo = addslashes($_POST['codigoCam']);
			
			$r= tentaSlcCamera($codigo);
	...
	}else if(isset($_POST['editar']) && $_POST['editar'] == 'Editar'){
		if(isset($_POST['cpf'])){
			...
			if(isset($_POST['resolucao']) && strlen($_POST['resolucao']) > 0){
				...
				$dados=array('codigoCam' => $codigoCam,'resolucao' =>$resolucao, 'cam_traseira' => $cam_traseira, 'cam_frontal' => $cam_frontal, 'flash' => $flash);
				
				$r=tentaEdtCamera($dados);
				$host = $_SERVER['HTTP_HOST'];
				$uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
				$extra = "index.php?r=$r";
				header("Location: http://$host$uri/$extra");
					...
					if(isset($_POST['peso']) && isset($_POST['radio']) && isset($_POST['viva_voz']) && isset($_POST['wi_fi'])){
						if(isset($_POST['multichip']) && isset($_POST['bluetooth']) && isset($_POST['mem_interna']) && isset($_POST['mem_externa'])){
							if(isset($_POST['mp3']) && isset($_POST['idioma1']) && isset($_POST['fabricante']) && isset($_POST['camera'])){
								...
								$dados=array('codigoAp' => $codigoAp ,'quantidade' =>$quantidade, 'modelo' => $modelo, 'display' => $display, 'preco_unitario' => $preco_unitario,
